...
class MyScene: WGScene {
    ...
    override init(size: CGSize, controller: WGDelegate) {
        super.init(size: size, controller: controller)
...
class TodayViewController: WGViewController {
    override func presentInitialScene() {
        self.gameView?.presentScene(MyScene(size: (gameView?.bounds.size)!, controller: self))
    }
}
